...
public class CupManager_backup : MonoBehaviour
{
    ...
    public void CupExport()
    {
        ...
        {
            //컵 자리 꽉 찼으면 팝업띄우고 탈출
            if (! Pickup.ChenkPickUp())
            {
                TextPopUp.OpenPopUp("픽업대가 모두 찼어요", -760f); return;
            }
            moving = true;
    ...
    {
        //썸넬드링크 만들기
        Drink drink = mixColor.MakeThumDrink(cupNum);
        Pickup.AddPickUpDrink(drink);
    ...
    public void ChangeCup(int _cupNum)
    {
        ...
        if (Resources.Load<Sprite>("Cup/Cap-" + cupNum) != null)
        {
            ...
            animator.SetBool("cap", true);
        }
        else animator.SetBool("cap", false);
